...
SCHEDULER_CONTAINER = os.environ.get("SCHEDULER_CONTAINER", "scheduler")
...
def find_scheduler(client):
    if not client:
        return None
    try:
        for c in client.containers.list(all=True):
            if SCHEDULER_CONTAINER in c.name:
                return c
    except Exception:
        pass
    return None
...
def get_status():
    ...
    container = find_scheduler(client)
    if container:
        result["scheduler_running"] = container.status == "running"
        result["uptime"] = container.attrs.get("State", {}).get("StartedAt")
...
def get_logs(
    ...
    if container:
        try:
            raw = container.logs(tail=limit * 4, timestamps=True).decode("utf-8", errors="replace")
...
async def stream_logs():
    async def generator():
        ...
        container = find_scheduler(client)
        if not container:
            yield f"data: {json.dumps({'error': 'scheduler not found'})}\n\n"
            return
        try:
    ...
    if "PAPER_TRADING" in body:
        ...
        if container:
            try:
                ...
                updated["_scheduler_restarted"] = True
            except Exception as exc:
                updated["_scheduler_restart_error"] = str(exc)
    return updated
